...
public class PolicyObject
{
    ...
    public List<Policy> ParsePolicies(TextAsset textAsset, string delimeter)
    {
        ...
        for (int i = 1; i < policyData.Length; i++)
        {
            ...
            int[] progress = Array.ConvertAll(policyDetails[3].Split(' '), int.Parse);
            int[] money = Array.ConvertAll(policyDetails[4].Split(' '), int.Parse);

            Policy policy = new Policy();
            policy.name = policyName;
            policy.cost = policyCost;

            policy.actions = new List<(string Name, int[] Values)>(); // maybe make int to list so can include capital and just make progress [0]
